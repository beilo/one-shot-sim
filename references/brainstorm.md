...
`task.py create` creates the default `prd.md`. Update that file with the current understanding before asking follow-up questions. In `one-shot-sim`, task creation belongs here, not in `plan`.
...
7. For complex tasks, defer `design.md` and `implement.md` to `plan`; `brainstorm` owns `prd.md`.
...
`brainstorm` does not create `design.md` or `implement.md`; it records requirements and open questions in `prd.md`. Complex tasks get `design.md` and `implement.md` during `plan`.
...
- Complex tasks have enough requirement clarity for `confirm` to decide whether research or planning can proceed.
