...
- `brainstorm`：读取并执行 `references/brainstorm.md`，创建或定位 planning task，并把需求写入 `prd.md`。
- `confirm`：确认 `prd.md` 和 research 是否足够进入规划；对外仍显示为 `confirm 执行确认`。
- `plan`：基于 `prd.md` 和 research 完成规划产物，并在自动挡中启动 task。
- `execute`：基于 active task 直接实现、检查和沉淀。
...
- `confirm` 已完成且已有 planning task：进入 `plan`。
- task 已启动且规划文档完整：进入 `execute`。
- `execute` 已完成且需要提交、归档、记录 journal：进入 `finish`。
- 发现旧 no-task 状态、无 task 的 `execute` / `finish` 状态，或历史状态要求跳过 task creation 时，停止并要求回到 `brainstorm` 创建 task；不做旧状态迁移。
...
- `confirm` 必须输出 `PRD`、`research`、`阻塞问题`、`下一步` 四项结论，并获得用户明确确认，才可进入 `plan`。
- 自动挡从用户确认后的 `plan` 开始连续推进。
- `plan` 完成后，主会话运行 `task.py start` 并直接进入 `execute`。默认自动挡和用户明确选择 `法拉利` / `自动挡` 本身都视为已批准执行。
...
- `confirm` 阶段进入前必须读取并执行 `references/confirm.md`。
...
<stage id="confirm" name="执行确认">
  ...
  <rules>
    读取并完整执行 `references/confirm.md`。
  </rules>

  <completion>
    - `references/confirm.md` 的完成标准已满足。
    - 输出 `<stage_complete id="confirm">`。
    - 下一阶段：`plan`。
    - 未获用户确认进入 `plan` 时，状态 `waiting_user`，不输出 stage_complete。
  </completion>
</stage>

## plan

<stage id="plan" name="规划">
